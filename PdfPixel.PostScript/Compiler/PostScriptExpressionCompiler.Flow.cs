...
public sealed partial class PostScriptExpressionCompiler
{
        ...
        switch (name)
        {
            ...
            case "ifelse":
            {
                ...
                if (falseProcExpr is ConstantExpression cf
                    ...
                    && ct.Value is PostScriptProcedure trueProc)
                {
                    if (!TryCompileBlockStack(trueProc, argsParam, parameterNames, out Stack<Expression> trueStack))
                    {
                        return false;
                    }

                    if (!TryCompileBlockStack(falseProc, argsParam, parameterNames, out Stack<Expression> falseStack))
                    {
                        return false;
                    }
    ...
    private static bool TryCompileBlockStack(PostScriptProcedure proc, ParameterExpression argsParam, IReadOnlyList<string> parameterNames, out Stack<Expression> resultStack)
    {
        ...
        foreach (PostScriptToken token in proc.Tokens)
        {
