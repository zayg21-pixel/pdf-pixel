...
internal static class ColorMath
{
    ...
    /// <summary>
    /// Convert profile connection space (PCS) values to D50-referenced XYZ.
    /// </summary>
    /// <param name="profile">Profile instance.</param>
    /// <param name="pcs">Evaluated PCS values.</param>
    /// <returns>D50-referenced XYZ vector.</returns>
    /// <exception cref="ArgumentException"></exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vector3? FromPcsToXyz(IccProfile profile, float[] pcs)
    {
        ...
        {
            return LabD50ToXyz(pcs[0] * 100f, pcs[1] * 255f - 128f, pcs[2] * 255f - 128f);
        }
        else if (profile?.Header?.Pcs == IccConstants.TypeXYZ)
        {
            return new Vector3(pcs[0], pcs[1], pcs[2]);
        }
        else
        {
            return default;
