...
namespace PdfReader.Fonts.PsFont
{
    internal static class Type1DictionaryToCffConverter
    {
        ...
        public static byte[] GenerateCffFontDataFromDictionary(PostScriptDictionary fontDictionary, PdfFontDescriptor descriptor)
        {
            ...
            Dictionary<PdfString, byte[]> type1CharStrings = PsFontDictionaryUtilities.GetCharStrings(fontDictionary);
            Dictionary<int, byte[]> type1Subrs = PsFontDictionaryUtilities.GetSubroutines(fontDictionary); // Source local subrs for flattening.
            float[] fontMatrix = PsFontDictionaryUtilities.GetFontMatrix(fontDictionary) ?? [0.001f, 0f, 0f, 0.001f, 0f, 0f];
            float[] effectiveFontBBox = PsFontDictionaryUtilities.GetFontBBox(fontDictionary) ?? [0f, 0f, 0f, 0f];

            string fontName = PsFontDictionaryUtilities.GetFontName(fontDictionary);
            ...
            PdfString[] encodingVector = PsFontDictionaryUtilities.GetEncodingVector(fontDictionary) ?? Array.Empty<PdfString>();
            ...
            int privateOffset)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                PsNumberConverter.EncodeDictFloat(ms, fontBBox[i]);
            }
            ms.WriteByte(5);
            ...
            for (int i = 0; i < 6; i++)
            {
                PsNumberConverter.EncodeDictFloat(ms, fontMatrix[i]);
            }
            ms.WriteByte(12);
            ms.WriteByte(7);

            // Encoding
            PsNumberConverter.EncodeDictInteger(ms, encodingOffset);
            ms.WriteByte(13);

            // Charset
            PsNumberConverter.EncodeDictInteger(ms, charsetOffset);
            ms.WriteByte(15);

            // CharStrings
            PsNumberConverter.EncodeDictInteger(ms, charStringsOffset);
            ms.WriteByte(17);

            // Private
            PsNumberConverter.EncodeDictInteger(ms, privateSize);
            PsNumberConverter.EncodeDictInteger(ms, privateOffset);
            ms.WriteByte(18);
