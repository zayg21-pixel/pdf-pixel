...
{
    private const float StandardGamma = 2.2f;
    ...
    public static IccProfile Parse(byte[] data)
    {
        ...
        BigEndianReader reader = new(data);
        IccProfile profile = new(data, IccProfileHeader.Read(reader));

        ParseTagDirectory(reader, profile);
        ParseSelectedTags(reader, profile);

        IccLutPipeline? lut = profile.A2BLut0 ?? profile.A2BLut1 ?? profile.A2BLut2;
    ...
    private static void ParseTagDirectory(BigEndianReader reader, IccProfile profile)
    {
        ...
        List<IccTagEntry> entries = new(Math.Max(0, tagCount));
    ...
    private static void ParseSelectedTags(BigEndianReader reader, IccProfile profile)
    {
        ...
        foreach (IccTagEntry tag in profile.Tags)
        {
    ...
    private static IccTrc ReadTrcPayload(BigEndianReader reader, int pos, out int payloadSize)
    {
        ...
        {
            return IccTrc.FromGamma(StandardGamma);
        }
        ...
        if (type == BigEndianReader.FourCC(IccConstants.TypeCurv))
        {
            ...
            var sampleCount = (int)Math.Min(int.MaxValue, count);
    ...
    private static IccTrc? ReadTrcType(BigEndianReader reader, IccTagEntry tag) => ReadTrcPayload(reader, tag.Offset, out _);
