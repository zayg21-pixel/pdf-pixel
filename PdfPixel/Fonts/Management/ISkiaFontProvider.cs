...
public interface ISkiaFontProvider : System.IDisposable
{
    ...
    /// <returns>A <see cref="PdfTypeface"/> that matches the requested standard font and style; or <c>null</c> if not found.</returns>
    PdfTypeface? GetStandardFont(PdfStandardFontName standardFont, SKFontStyle style, string? unicode, float? width);
    ...
    /// <param name="unicode">Optional unicode text to validate glyph availability.</param>
    /// <param name="width">Optional horizontal scale hint (1.0 = normal).</param>
    /// <returns>A <see cref="PdfTypeface"/> that matches the requested font; or <c>null</c> if not found.</returns>
    PdfTypeface GetFont(string? name, SKFontStyle style, string? unicode, float? width);
}
