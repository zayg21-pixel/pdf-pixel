...
internal readonly struct Jbig2TextRegionHuffmanFlags
{
    ...
    /// <summary>Huffman table selection for first S (SBHUFFFS, bits 0–1).</summary>
    public int FsSelection { get; }

    /// <summary>Huffman table selection for delta S (SBHUFFDS, bits 2–3).</summary>
    public int DsSelection { get; }

    /// <summary>Huffman table selection for delta T (SBHUFFDT, bits 4–5).</summary>
    public int DtSelection { get; }

    /// <summary>Huffman table selection for refinement delta width (SBHUFFRDW, bits 6–7).</summary>
    public int RefinementDwSelection { get; }

    /// <summary>Huffman table selection for refinement delta height (SBHUFFRDH, bits 8–9).</summary>
    public int RefinementDhSelection { get; }

    /// <summary>Huffman table selection for refinement delta X (SBHUFFRDX, bits 10–11).</summary>
    public int RefinementDxSelection { get; }

    /// <summary>Huffman table selection for refinement delta Y (SBHUFFRDY, bits 12–13).</summary>
    public int RefinementDySelection { get; }

    /// <summary>Refinement bitmap size coding selector (SBHUFFRSIZE, bit 14).</summary>
