...
internal static class Jbig2HuffmanPlacementDecoder
{
    ...
    internal static Jbig2HuffmanTable DecodeSymbolIdTable(Jbig2HuffmanDecoder reader, int symbolCount)
    {
        ...
        var runCodeLines = new Jbig2HuffmanTable.HuffmanLine[35];
        for (int i = 0; i <= 34; i++)
        {
            int codeLength = reader.ReadBits(4);
            runCodeLines[i] = new Jbig2HuffmanTable.HuffmanLine(i, 0, codeLength);
        }

        var runCodesTable = Jbig2HuffmanTable.Build(runCodeLines);

        // Decode symbol ID code lengths using RUNCODEs
        var symbolLines = new List<Jbig2HuffmanTable.HuffmanLine>(symbolCount);
        int symbolIndex = 0;
        ...
        while (symbolIndex < symbolCount)
        {
            ...
            if (codeLength >= 32)
            {
                ...
                {
                    symbolLines.Add(new Jbig2HuffmanTable.HuffmanLine(symbolIndex, 0, repeatedLength));
                    symbolIndex++;
                }
            }
            else
            {
                symbolLines.Add(new Jbig2HuffmanTable.HuffmanLine(symbolIndex, 0, codeLength));
                symbolIndex++;
            }
        }
