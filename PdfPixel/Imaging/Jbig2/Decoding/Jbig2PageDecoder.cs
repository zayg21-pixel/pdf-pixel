...
internal sealed class Jbig2PageDecoder
{
    ...
    public Jbig2PageDecoder(ILogger logger)
    {
    ...
    /// </param>
    /// <param name="cancellationToken">Token to cancel long-running decode operations.</param>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (width <= 0 || height <= 0)
        {
            _logger.LogError("JBIG2 invalid page dimensions: {Width}x{Height}.", width, height);
            return null;
        }
        ...
        ProcessSegments(pageSegments, pageData, pageBitmap, _cache, cancellationToken);
    ...
    private void ProcessSegments(List<Jbig2SegmentHeader> segments, ReadOnlySpan<byte> data, Jbig2Bitmap pageBitmap, Jbig2SegmentCache cache, CancellationToken cancellationToken = default)
    {
        foreach (var segment in segments)
        {
            ...
            switch (segment.Type)
            {
                ...
                default:
                    _logger.LogDebug("JBIG2 skipping segment type {Type} (number={Number}).", segment.Type, segment.SegmentNumber);
                    break;
            }
        }
    }
    ...
    private void ProcessHalftoneRegion(Jbig2SegmentHeader segment, ReadOnlySpan<byte> segmentData, Jbig2Bitmap pageBitmap, Jbig2SegmentCache cache)
    {
        ...
        {
            _logger.LogWarning("JBIG2 halftone region segment {SegmentNumber} has no referred pattern dictionary.", segment.SegmentNumber);
            return;
        }
